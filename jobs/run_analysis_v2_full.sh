...
#SBATCH --output=/home/$USER/slurm_analysis_v2_%j.out

# ============================================================
# Full V2 Analysis Pipeline
# Phase 1: Core analysis (tokenize, ngram, CLAP, FAD, musico, vuln)
# Phase 2: Supplementary analyses (S1-S7)
...
for f in /home/$USER/tok_*.py; do
    base=$(echo $f | sed 's|.*/tok_||')
    cp "$f" "$REPO/src/tokenization/$base"
done

cd $REPO

# Ensure v2 has needed dirs
mkdir -p $V2_DIR/{analysis,embeddings,tokens_generated,logs,manifests}

# Copy sampling manifest from v1 (needed by analysis scripts)
cp $V1_DIR/manifests/sampling_manifest.csv $V2_DIR/manifests/ 2>/dev/null || \
cp $V1_DIR/manifests/tracks_selected.csv $V2_DIR/manifests/sampling_manifest.csv 2>/dev/null || true
cp $V1_DIR/manifests/tracks_selected.csv $V2_DIR/manifests/ 2>/dev/null || true

# Symlink catalog tokens and masters from v1
ln -sf $V1_DIR/tokens_catalog $V2_DIR/tokens_catalog 2>/dev/null || true
ln -sf $V1_DIR/masters_clean $V2_DIR/masters_clean 2>/dev/null || true

echo "============================================================"
echo "PHASE 1: Core V2 Analysis Pipeline"
echo "Started: $(date)"
echo "============================================================"

# Step 1: Tokenize v2 generated audio
echo "[$(date)] Step 1: Tokenizing generated audio..."
python src/tokenization/tokenize_generated.py \
    --config "$CONFIG" --run_id "$RUN_ID" --run_dir "$V2_DIR"

# Step 2: N-gram search
echo "[$(date)] Step 2: N-gram search..."
python src/analysis/ngram_search.py \
    --config "$CONFIG" --run_id "$RUN_ID" --run_dir "$V2_DIR"

# Step 3: N-gram statistics
echo "[$(date)] Step 3: N-gram statistics..."
...
for drop in signals:
    ...
    # Rank correlation with full score
...
# ============================================================
# S5: Genre-controlled Z-scores (from vulnerability scores)
# ============================================================
...
# ============================================================
# C5: Training Loss vs Vulnerability (just note the values)
# ============================================================
...
# ============================================================
print("\n=== C7: Effect Size Comparison ===")
# Load v1 supplementary effect sizes if available
...
# Try to compute v1 effect sizes on the fly
