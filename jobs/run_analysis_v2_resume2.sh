...
#SBATCH --job-name=v2_res2
#SBATCH --output=/home/$USER/slurm_analysis_v2_res2_%j.out

# Resume V2 analysis from Step 6 (FAD) onward
# Steps 1-5 completed
...
echo "============================================================"
echo "RESUMING V2 Analysis from Step 6"
echo "Started: $(date)"
echo "============================================================"
...
for a in common_artists:
    ...
    if v1_v is not None and v2_v is not None:
        delta_rows.append({
            "artist_id": a, "artist_name": v1_vuln[a].get("artist_name", ""),
            "genre": v1_vuln[a].get("genre", ""),
            "v1_vulnerability": round(v1_v, 6), "v2_vulnerability": round(v2_v, 6),
...
with open(f"{OUT}/full_comparison_summary.json", "w") as f:
    json.dump({
        "aggregate": agg, "clap_gap": clap_comp, "ngram": ngram_comp,
        "loss": loss_comp, "rank_stability": rank_stability, "effect_sizes": effect_comp,
        "per_artist_delta": {"n_gained": gained, "n_lost": lost,
